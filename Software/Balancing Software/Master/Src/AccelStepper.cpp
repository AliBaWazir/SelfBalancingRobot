...
{   
    ...
    setOutputPins(_direction ? 0x2 : 0x0); // Set direction first else get rogue pulses
    setOutputPins(_direction ? 0x3 : 0x1); // step HIGH
    // Caution 200ns setup time 
    // Delay the minimum allowed pulse width
    delayMicroseconds(_minPulseWidth);
    setOutputPins(_direction ? 0x2 : 0x0); // step LOW
